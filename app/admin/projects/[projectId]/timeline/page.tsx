...
import { ArrowLeft, RotateCcw, Eye, Clock, AlertCircle, CheckCircle2 } from 'lucide-react';
...
export default function AdminProjectTimelinePage() {
  ...
  const getChangeIcon = (changeType?: string) => {
    switch (changeType) {
      ...
      case 'status_changed':
        return <Badge className="text-xs bg-teal-600 text-white">Status</Badge>;
      default:
  ...
  const getApplianceCount = (rooms: { appliances?: unknown[] }[] = []) => {
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-slate-400 mt-1">
            {(project as { client_info?: { name?: string } } | null)?.client_info?.name || 'Untitled'} - {history.length} edits
          </p>
        </div>
      </div>
          ...
          <CardContent className="p-6">
            <div className="grid grid-cols-4 gap-4">
              <div>
                ...
                <div className="text-2xl font-bold text-white">{(project as { rooms?: unknown[] }).rooms?.length || 0}</div>
              </div>
              <div>
                <div className="text-sm text-slate-400">Appliances</div>
                <div className="text-2xl font-bold text-teal-400">{getApplianceCount((project as { rooms?: { appliances?: unknown[] }[] }).rooms || [])}</div>
              </div>
              <div>
                <div className="text-sm text-slate-400">Total Cost</div>
                <div className="text-2xl font-bold text-white">
                  ₹{((project as { total_cost?: number }).total_cost || 0).toLocaleString()}
                </div>
              </div>
              <div>
                ...
                <Badge
                  className={`${
                    (project as { status?: string }).status === 'completed'
                  ...
                  variant="outline"
                >
                  {(project as { status?: string }).status || 'draft'}
                </Badge>
              </div>
            </div>
          </CardContent>
              ...
              <Card className="border-white/10 bg-slate-900/50">
                ...
                <CardContent className="space-y-3 text-sm">
                  ...
                  <div>
                    ...
                    <div className="text-teal-400 font-semibold mt-1">{getApplianceCount(selectedEntry.rooms as { appliances?: unknown[] }[] || [])}</div>
                  </div>
                  <div>
        ...
        <Card className="border-white/10 bg-slate-900/50">
          ...
          <CardContent className="space-y-4 max-h-96 overflow-y-auto">
            ...
            <div>
              ...
              <div className="space-y-2">
                {selectedEntry.rooms?.slice(0, 5).map((room, idx) => (
